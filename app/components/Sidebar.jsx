	...
	<div className={`${nunito.className} ${'fixed top-[56px] left-0 h-full backdrop-blur-lg bg-white/60 shadow-lg duration-200 z-20'}`}>
		<div id='sidebar' className={`${nunito.className} ${'w-0 overflow-y-auto lg:max-h-[calc(100%_-_72px)] max-h-[calc(100%_-_56px)] duration-200 z-11'}`}>
			...
			<div className='flex flex-col justify-between px-2 my-6 space-y-2'>
				...
				{ !session ? <div className='flex flex-col space-y-2'>
					<Link href='' className='sidebar-button hover:bg-[#e6f2ff] hover:text-Blue bg-[#c7e2ff]/60 backdrop-blur-lg text-Blue rounded-lg overflow-hidden duration-100'>
						<div className='sidebar-button'>
							<div className='sidebar-button sidebar-button-toggle flex items-center space-x-3 px-6 py-2' onClick={() => {signIn()}}>
								<GoSignIn className='text-xl sidebar-button-icon' />
								<p className='sidebar-btn-text'>Sign In</p>
							</div>
						</div>
					</Link>
				</div> : 
					...
					<Link href='' className='sidebar-button bg-[#ff1c1c1c]/20 backdrop-blur-lg text-Red rounded-lg overflow-hidden duration-100'>
						<div className='sidebar-button'>
							<div className='sidebar-button sidebar-button-toggle flex items-center space-x-3 px-6 py-2' onClick={() => {signOut()}}>
								<GoSignOut className='text-xl sidebar-button-icon' />
